...
function interpretar(data, status, codigo) {
  ...
  const listadoValido = data.Listado.filter(t => t && typeof t === 'object' && NG.valido(NG.codigo(t.CodigoExterno || t.Codigo)));
  if (!codigo) {
    if (data.Listado.length && !listadoValido.length) {
      throw new ConsultaError('INVALID_RESPONSE', 'Mercado Público devolvió información incompleta. Vuelve a intentar.');
    }
    if (listadoValido.length !== data.Listado.length) {
      return { ...data, Cantidad: listadoValido.length, Listado: listadoValido };
    }
    return data;
  }
  if (listadoValido.length !== data.Listado.length) {
  ...
  if (data.Listado.length !== 1 || NG.codigo(data.Listado[0].CodigoExterno || data.Listado[0].Codigo) !== codigo) {
