...
function apiDate(v){const [y,m,d]=String(v).split("-");return d&&m&&y?\`\${d}\${m}\${y}\`:""}
...
function scoreNG(o){
  ...
  let s=0;const hits=[];
  for(const p of PERFIL.palabras){if(text.includes(norm(p))){s+=7;hits.push(p)}}
  for(const z of PERFIL.zonas){if(text.includes(norm(z))){s+=18;hits.push(z)}}
  const d=daysUntil(o.fechaCierre);if(d!==null&&d>1)s+=3;
  return {score:Math.min(100,s),hits:[...new Set(hits)].slice(0,5)};
}
...
function openAnalysis(codigo,tab="resumen"){location.href=\`/analisis.html?codigo=\${encodeURIComponent(codigo)}#\${tab}\`}
function recordState(o){
  ...
  const next={codigo:o.codigo,nombre:o.nombre||"",estado:o.estado||"",comprador:o.comprador||"",fechaCierre:o.fechaCierre||null,ngAdjudicada:!!o.ngAdjudicada,proveedoresAdjudicados:o.proveedoresAdjudicados||[],numeroOferentes:o.numeroOferentes??null,actualizadoEn:new Date().toISOString()};
  setHistory([next,...arr.filter(x=>x.codigo!==o.codigo)]);
}
function normalizeResponse(j){
  const list=Array.isArray(j?.data?.Listado)?j.data.Listado:[];const analyses=Array.isArray(j?.analisis)?j.analisis:[];
  return list.map((t,i)=>({
    codigo:t.CodigoExterno||t.Codigo||"",nombre:t.Nombre||"",descripcion:t.Descripcion||"",estado:t.Estado||"",
    comprador:t?.Comprador?.NombreOrganismo||t?.Comprador?.NombreUnidad||"",region:t?.Comprador?.RegionUnidad||t?.Comprador?.Region||"",
    comuna:t?.Comprador?.ComunaUnidad||t?.Comprador?.Comuna||"",fechaCierre:t?.Fechas?.FechaCierre||t.FechaCierre||null,
    montoEstimado:t.MontoEstimado||t.Monto||t.Presupuesto||t.ValorEstimado||null,...(analyses[i]||{})
  }));
}
...
async function searchByDate(){
  ...
  try{
    ...
    oportunidades=normalizeResponse(j).map(o=>({...o,_ng:scoreNG(o)}));oportunidades.forEach(recordState);
...
function filteredOpportunities(){
  ...
  return oportunidades.filter(o=>{
    ...
    if(q&&!text.includes(q))return false;if(zone&&!text.includes(zone))return false;if(only&&(o._ng?.score||0)<14)return false;return true;
  }).sort((a,b)=>(b._ng?.score||0)-(a._ng?.score||0));
}
  ...
  root.innerHTML=list.slice(0,120).map(o=>{
    ...
    return \`<article class="opportunity-card \${ng.score>=14?"recommended":""}">
      <div class="opportunity-main">
        <div class="row"><span class="mini">\${esc(o.codigo||"Sin ID")}</span>\${ng.score>=14?'<span class="status ok">Sugerida NG</span>':""}\${urgent?'<span class="status danger">Cierre próximo</span>':""}</div>
        <h3>\${esc(o.nombre||"Sin nombre")}</h3>
        <div class="op-meta"><span>\${esc(o.comprador||"Comprador no informado")}</span><span>\${esc([o.comuna,o.region].filter(Boolean).join(" · ")||"Ubicación no informada")}</span><span>\${esc(formatMoney(o.montoEstimado))}</span><span>Cierre: \${esc(formatDate(o.fechaCierre))}</span></div>
        \${ng.hits.length?\`<div class="match-line">Calce detectado: \${esc(ng.hits.join(" · "))}</div>\`:""}
      </div>
      <div class="opportunity-actions">
        <button class="primary" data-action="analizar" data-code="\${esc(o.codigo)}">Analizar</button>
        <button class="secondary" data-action="buyer" data-code="\${esc(o.codigo)}">Ver comprador</button>
        <button class="ghost" data-action="save" data-code="\${esc(o.codigo)}">\${isSaved(o.codigo)?"Guardada ✓":"Guardar"}</button>
      </div>
    </article>\`;
  }).join("");
}
...
function refreshBusiness(code){
  const btn=document.querySelector(\`[data-refresh="\${code}"]\`);if(btn){btn.disabled=true;btn.textContent="Revisando…"}
  fetchMP({codigo:code}).then(j=>{
    const o=normalizeResponse(j)[0];if(!o)throw new Error("Sin datos");o._ng=scoreNG(o);recordState(o);
    const arr=saved(),i=arr.findIndex(x=>x.codigo===code);if(i>=0)arr[i]={...arr[i],nombre:o.nombre,comprador:o.comprador,estado:o.estado,fechaCierre:o.fechaCierre};setSaved(arr);refreshDashboard();
...
function renderCompetition(){
  ...
  for(const x of h){if(x.numeroOferentes)withN++;for(const p of x.proveedoresAdjudicados||[]){const key=cleanRut(p.rut)||p.nombre;if(key)adjud.set(key,p)}}
  $("compAdjudicatarios").textContent=adjud.size;$("compOferentes").textContent=withN;$("compHistorial").textContent=h.length;
  const names=[...adjud.values()].slice(0,12);
...
$("perfilChips").innerHTML=[...PERFIL.zonas,"Obras civiles","Electricidad","Estructuras metálicas","Climatización","Pintura","Mobiliario"].map(x=>\`<span class="profile-chip">\${esc(x)}</span>\`).join("");
...
$("resultadoOportunidades").addEventListener("click",e=>{const b=e.target.closest("button[data-action]");if(!b)return;const code=b.dataset.code,o=oportunidades.find(x=>x.codigo===code);if(!o)return;if(b.dataset.action==="analizar")openAnalysis(code);if(b.dataset.action==="buyer")selectBuyer(code);if(b.dataset.action==="save"){isSaved(code)?removeBusiness(code):addBusiness(o)}});
$("listaNegocios").addEventListener("click",e=>{const b=e.target.closest("button[data-business]");if(!b)return;const code=b.dataset.code;if(b.dataset.business==="open")openAnalysis(code);if(b.dataset.business==="refresh")refreshBusiness(code);if(b.dataset.business==="remove")removeBusiness(code)});
